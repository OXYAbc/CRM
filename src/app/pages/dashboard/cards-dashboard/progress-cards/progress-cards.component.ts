...
export class ProgressCardComponent {
  ...
  levelOfComplited(project: Project) {
    ...
    const toDoPercent: number =
      (project.tasks.filter((res: Task) => res.stage == 'toDo').length /
        mainLength) *
      100;
    const inProgressPercent: number =
      (project.tasks.filter((res: Task) => res.stage == 'inProgress').length /
        mainLength) *
      100;
    const inReviewPercent: number =
      (project.tasks.filter((res: Task) => res.stage == 'inReview').length /
        mainLength) *
      100;
    const donePercent: number =
      (project.tasks.filter((res: Task) => res.stage == 'done').length /
        mainLength) *
      100;
